...
当前 `/` 首页除了原始终端日志外，还会优先展示从 `~/.codex/sessions/*.jsonl` 提取的结构化时间线（任务、推理、工具调用、命令输出、最终答复）；如果无法关联到结构化会话，则自动回退到 tmux 日志视图。

手机端“更多”页已支持三档执行模式：`巡检模式`（只读 + 谨慎批准）、`开发模式`（工作区写入 + 按需批准）、`全自动`（危险但省心）。点“恢复共享会话”会按当前选中的模式启动；如果共享会话已存在且模式不同，会自动按新模式重建。

同时支持为当前项目保存两条自定义快捷任务，进入聊天页后会和默认快捷按钮一起显示。
